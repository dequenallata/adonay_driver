...
import 'package:adonay_driver/functions/audio_servce.dart';
import 'package:adonay_driver/main.dart';
import 'package:adonay_driver/models/code_whats_app.dart';
import 'package:flutter_tts/flutter_tts.dart';
import 'package:intl/intl.dart';
import 'package:adonay_driver/models/deuda_model.dart';
import 'package:adonay_driver/pages/chatPage/chat_page.dart';
import 'package:adonay_driver/translation/translation.dart';
...
import 'package:adonay_driver/utils/location_manager.dart';
...
//base url
// String url = 'https://15deabril.macrobyte.site/public/'; //add '/' at the end of the url as 'https://url.com/'
String url = 'https://adonay.macrobyte.cloud/';
String mapkey = Platform.isAndroid ? 'AIzaSyA0Jl2tjqouHdhCzNF8nogi381QHIq9c_w' : 'AIzaSyA0Jl2tjqouHdhCzNF8nogi381QHIq9c_w';
String mapStyle = '';
String mapType = '';



// Función para obtener la ruta desde OSRM
// Future<List<fmlt.LatLng>> getRouteFromOSRM(double startLat, double startLng, double endLat, double endLng) async {
//     final response = await http.get(Uri.parse('https://router.project-osrm.org/route/v1/driving/$startLng,$startLat;$endLng,$endLat?overview=full&geometries=geojson'));

//     if (response.statusCode == 200) {
//       final data = jsonDecode(response.body);
//       final List<dynamic> coordinates = data['routes'][0]['geometry']['coordinates'];

//       // Convertir las coordenadas a una lista de fmlt.LatLng
...
//     } else {
//       throw Exception('Error al obtener la ruta de OSRM');
//     }
//   }

 Future<void> unlockAccount(BuildContext context, String driverId) async {
  var dio = Dio();
  var data = FormData.fromMap({
    'driver_id': int.parse(driverId), // Usa el id que viene del usuario
  });

  try {
    // Realiza la solicitud a la API
    var response = await dio.request(
      'https://adonay.macrobyte.cloud/api/desbloquear/cuenta',
      options: Options(
        method: 'POST',
      ),
      data: data,
    ...
    // Si la respuesta es exitosa y no hay error 422
    if (response.statusCode == 200) {
      Navigator.of(context).pop(); // Cierra la alerta activa
      showProfessionalAlert(context, "Éxito", "Cuenta desbloqueada correctamente.", true, driverId);
    }

  } on DioError catch (e) {
    if (e.response?.statusCode == 422) {
      // Si el error es 422, cierra la alerta existente
      Navigator.of(context).pop(); // Cierra la alerta activa

      // Extrae el mensaje de error desde "details"
      String errorMessage = e.response?.data['details'] ?? "Error desconocido";

      // Muestra otra alerta con el mensaje del backend
      showProfessionalAlert(context, "Error", errorMessage, false, driverId);
    } else {
      print(e);
      // Maneja otros errores de red o problemas
    }
  } catch (e) {
    print(e);
    // Captura otros errores no relacionados con Dio
  }
}

Future<Map<String, dynamic>> obtenerDeuda(String driverId) async {
  final response = await http.post(
    Uri.parse("https://adonay.macrobyte.cloud/api/deuda/driver/inicio"),
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception("Error al obtener datos de la deuda");
  }
}


void showAlertSuccess( BuildContext context, String title, String message, bool isSuccess) {
    showDialog(
      context: context,
...
void showProfessionalAlert( BuildContext context, String title, String message, bool isSuccess, String driverId) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
  ...
  bool mensajeRecogidaReproducido = false;


double calcularDistancia(fmlt.LatLng origen, fmlt.LatLng destino, String flag) {
  const fmlt.Distance distancia = fmlt.Distance();
  double metros = distancia.as(fmlt.LengthUnit.Meter, origen, destino);

  if (flag == 'RECOGIDA' && metros <= 100 && !mensajeRecogidaReproducido) {
    final FlutterTts flutterTts = FlutterTts();
    flutterTts.setLanguage("es-ES");
    flutterTts.speak('Estás cerca del punto de recogida');
    mensajeRecogidaReproducido = true; // Evita que se repita
  }
...
Future<DeudaResponse?> obtenerDeudas(int driverId) async {
  final url = Uri.parse('https://adonay.macrobyte.cloud/api/deuda/driver');
  final response = await http.post(
    url,
    headers: {"Content-Type": "application/json"},
  ...
  if (response.statusCode == 200) {
    ...
    return DeudaResponse.fromJson(jsonResponse);
  } else {
    return null;
  }
}
...
Future<String?> getPlaceName(double lat, double long) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (jsonData.containsKey('display_name')) {
      return jsonData['name']; // Retorna la dirección formateada
    } else {
...
// Future<void> setDriverOnlineStatus(bool status) async {
//   SharedPreferences prefs = await SharedPreferences.getInstance();
//   String? driverId = prefs.getString('id'); // Obtén el ID del conductor

//   if (driverId != null) {
//     final DatabaseReference ref =FirebaseDatabase.instance.ref('drivers/driver_$driverId');
//     await ref.update({'app_abierta': status});
//   }
// }
...
verifyUser(String number, int login, String password, String email, isOtp, forgot) async {
  dynamic val;
  try {
...
Future<dynamic> getUserDetails() async {
  // Define un periodo de tiempo mínimo entre ejecuciones
  const debounceDuration = Duration(seconds: 2);

  // Verifica si el método fue llamado antes y si han pasado al menos 2 segundos
  if (_lastExecutionTime != null && DateTime.now().difference(_lastExecutionTime!) < debounceDuration) {
          ...
          if (tripStops.isNotEmpty) {
            for (var i = 0; i < tripStops.length; i++) {
              addressList.add(AddressList(
                  ...
                  // pickup: false,
                  address: tripStops[i]['address'],
                  latlng: LatLng(tripStops[i]['latitude'], tripStops[i]['longitude']),
                  name: tripStops[i]['poc_name'],
                  number: tripStops[i]['poc_mobile'],
                  instructions: tripStops[i]['poc_instruction'],
                ),
...
String? errorMessageValidateMobile;
Future<bool> loginWithWhatsapp(String number) async {
  bool? resp;
  try {
    final response = await http.get(
      Uri.parse('${url}api/usuario/validar/?telefono=$number'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      if (res['id'] != null) {
        ...
        errorMessageValidateMobile = res['error'];
        resp = false;
      }
    }
    return resp!;
  ...
  Future<CodeWhatsApp?> sendCodeWhatsApp(
    ...
    try {
      String url =
          "https://adonay.macrobyte.cloud/api/whatsapp/code?telefono=$number&code=$code";
...
sound() async {
  Timer.periodic(const Duration(seconds: 1), (timer) async {
    if (duration > 0.0 && driverReq['accepted_at'] == null &&
      ...
      valueNotifierHome.incrementNotifier();
        ...
        driverReq['accepted_at'] == null &&  duration <= 0.0) {
      timer.cancel();
      if (audioPlayers.state != PlayerState.stopped) {
        audioPlayers.stop();
        // audioPlayers.dispose();
  ...
  try {
    ...
    if (response.statusCode == 200) {
      if (jsonDecode(response.body)['message'] == 'driver_arrived') {
        ...
        waitingTime = 0;
        await getUserDetails();
        // FirebaseDatabase.instance.ref('requests').child(
        //   driverReq['id']).update(
        //     {
...
endTrip() async {
  dynamic result;
  try {
    await requestDetailsUpdate(double.parse(heading.toString()),  currentPositionNew!.latitude, currentPositionNew!.longitude);
    var dropAddress = await geoCoding(currentPositionNew!.latitude, currentPositionNew!.longitude);
    ...
    if (response.statusCode == 200) {
      await getUserDetails();
      // FirebaseDatabase.instance.ref('requests').child(reqId).update( {'is_completed': true, 'modified_by_driver': ServerValue.timestamp});
      totalDistance = null;
      lastLat = null;
      fmpoly.clear();
      lastLong = null;
...
// userRating(String feedback) async {
//   dynamic result;
//   try {
...
List chatList = [];
DateTime? _lastExecutionTimeMessage;
// getCurrentMessages() async {
//   const debounceDurationMessage = Duration(seconds: 2);
//   if (_lastExecutionTimeMessage != null && DateTime.now().difference(_lastExecutionTimeMessage!) < debounceDurationMessage) {
//     return;
//   }
//   _lastExecutionTimeMessage = DateTime.now();
//   dynamic result;
//   try {
//     var response = await http.get(
...
// getData()async{
//   SharedPreferences prefs = await SharedPreferences.getInstance();
//   if (prefs.getBool('openChat') != null ) {
//     Navigator.push<void>(
...
// sendMessageAudio() async {
//   var headers = {
//   'user_id': '917',
//   'request_id': 'dc2080e3-0d09-4de9-8237-790554759fd7',
//   'audio': 'assets/Calle-Haiti-36.mp3'
// };
//   dynamic result;
//   try {
//     var response = await http.post(
//       Uri.parse('https://15deabril.macrobyte.site/api/audio/chat'),
//         headers: headers,
//         );
//     if (response.statusCode == 200) {
//       getCurrentMessages();
...
Future<void> notifyBocina() async {
 SharedPreferences prefs = await SharedPreferences.getInstance();
  final rId = prefs.getString('requestId');
  const url = 'https://adonay.macrobyte.cloud/api/bocina';
  
  try {
    final response = await http.post(
      Uri.parse(url),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'request_id': rId}),
    );
...
waitingAfterStart() async {
  ...
  rideTimer = Timer.periodic(const Duration(seconds: 60), (timer) async {
    if (currentRidePosition == null &&
        driverReq['is_completed'] == 0 &&
        driverReq['is_trip_start'] == 1) {
      currentRidePosition = centerPosition;
    } else if (currentRidePosition != null &&
        driverReq['is_completed'] == 0 &&
        driverReq['is_trip_start'] == 1) {
      var dist = await calculateIdleDistance(currentRidePosition.latitude,
          currentRidePosition.longitude, centerPosition!.latitude, centerPosition!.longitude);
      if (dist < 150) {
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ? geolocs.AndroidSettings(
        ...
        foregroundNotificationConfig:
          ...
          notificationText:
              "Radiomóvil 15 de abril Tarija Driver will continue to receive your location in background",
